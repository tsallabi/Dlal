...
partner.post('/item/:id/unavailable', async (c) => {
  ...
  await notify(db, it.user_id, `منتج غير متوفر في طلبك ${it.code}`, `"${it.title_ar}" نفد عند المورد. سيتواصل معك فريق تالين لاختيار بديل أو استرجاع قيمته.`, `/orders/${it.code}`);
