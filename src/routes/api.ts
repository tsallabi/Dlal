...
api.post('/source/stats', async (c) => {
  ...
  return c.json({ totals: { products: tot?.n ?? 0, active: tot?.a ?? 0, from1688: tot?.s ?? 0, providerCalls: src?.n ?? 0, outOfStock: tot?.oos ?? 0, chineseTitles: cn?.n ?? 0, heldDraft: tot?.dr ?? 0 }, categories: cats, jobs });
});
