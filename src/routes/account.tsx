...
acct.get('/tickets/:code', async (c) => {
  ...
  return shell(c, 'tickets', `${t.code} — ${t.subject}`, (
    <>
      ...
      <div class="chat">{msgs.results.map(m => <div class={`msg ${m.is_staff ? 'staff' : 'me'}`}><div class="who">{m.is_staff ? `فريق دلال — ${m.name ?? ''}` : 'أنتِ'} · {timeAgo(m.created_at)}</div><div>{m.body}</div>{m.image_url && <a href={m.image_url} target="_blank">📷 صورة</a>}</div>)}</div>
