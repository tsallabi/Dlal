...
// عنوان إنجليزي خالص: الإضافة تقرأ document.title لزائر غير مسجّل فيصل بالإنجليزية («Front zipper sports bra…»).
// كان يمرّ من `Translator.t` كما هو (لا صيني فيه) ويدخل الرف نشطًا — أول منتج من الجلب المجاني ٢٤/٠٩/٢٦.
export const hasArabic = (t: string | null | undefined) => !!t && /[\u0621-\u064A]/.test(t);
export const enTitle = (t: string | null | undefined) => !!t && !hasArabic(t) && !hasCJK(t) && /[A-Za-z]{3}/.test(t);
...
const SYS_TITLE = 'أنت مترجم لمتجر أزياء عربي. حوّل عنوان منتج من موقع 1688 (صيني أو إنجليزي، محشو بكلمات مفتاحية) إلى عنوان منتج عربي قصير وطبيعي من 5 إلى 14 كلمة يصف المنتج للزبون. احذف عبارات مثل "تجارة خارجية"، "عبر الحدود"، "جديد 2025"، "بالجملة"، "موديل جديد". اذكر ما يميّز هذه القطعة تحديدًا (الخامة أو المقاس أو عدد القطع أو الاستعمال) ولا تكتفِ بوصف عام يصلح لعشرات المنتجات. أجب بالعنوان العربي فقط، بلا شرح ولا علامات اقتباس.';
...
export class Translator {
  ...
  async t(text: string | null | undefined, kind: 'text' | 'attr' | 'title' = 'text', hintEn?: string | null, extra?: string): Promise<string | null> {
    ...
    if (!text || (!hasCJK(text) && !mixedScript(text) && !en && !(kind === 'title' && enTitle(text)))) return text ?? null;
    const k = norm(text);
