...
def svg(anim=True, size='', bust=False):
    out = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 200" {size} role="img" aria-label="هدهد HUDHUD">{STYLE if anim else ''}
<defs>
...
def lockup(kind='ar', dark=False, h=76):
    ...
    if kind == 'ar':
        text = f'<span class="t"><b style="color:{ink}">هدهد <em>الصين</em></b><i style="color:{ink}">HUDHUD</i></span>'
    else:
        text = f'<span class="t"><b class="lat" style="color:{ink}">HUDHUD</b><i class="ar" style="color:{ink}">هدهد <em>الصين</em></i></span>'
...
<div class="card"><h2>الخيار ٢ — HUDHUD أولًا وتحته «هدهد الصين»</h2>
...
<figure><span class="tab">{icon()} هدهد الصين | HUDHUD</span><figcaption>في تبويب كروم</figcaption></figure>
</div></div>
